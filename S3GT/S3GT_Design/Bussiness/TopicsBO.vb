...
Public Class TopicsBO
    ...
    Function is_BLANK(ByVal text As String) As Boolean
        ...
        Return flag
    End Function
    Function is_DupcateTopic(ByVal table As Data.DataTable, ByVal row As Data.DataRow) As Boolean
        Dim flag As Boolean = False
        If (table Is Nothing) Then Return False
        For Each item As Data.DataRow In table.Rows
            'GiangNVT modified
            If LCase(item.Item(Constants_HaoLT.TOPIC_NAME)) = LCase(row.Item(Constants_HaoLT.TOPIC_NAME)) And LCase(item.Item(Constants_HaoLT.TOPIC_GROUP_NAME)) = LCase(row.Item(Constants_HaoLT.TOPIC_GROUP_NAME)) Then
                Return True
            End If
        Next
        Return flag
    ...
    Public Function GetTopicByTopicNameGroupName(ByVal groupName As String, ByVal topicName As String) As DataTable
        ...
        objTopicDAO = New TopicDAO()
        'TopicTable.DefaultView.RowFilter = ""
        Return objTopicDAO.GetTopicByTopicNameGroupName(groupName, topicName)
